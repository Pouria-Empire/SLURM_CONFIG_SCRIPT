...
sudo mkdir -p /run/slurmctld
sudo chown slurm:slurm /run/slurmctld
sudo chmod 755 /run/slurmctld
...
# `Can't open PID ...` is not an improtant error
...
#
# Remember not to ADD Controller as a NodeName
# COMPUTE NODES
NodeName=worker NodeAddr=213.233.184.169 CPUs=2 Boards=1 SocketsPerBoard=1 CoresPerSocket=2 ThreadsPerCore=1 RealMemory=9966 State=UNKNOWN
#NodeName=compute-slurm2 NodeAddr=192.168.56.103 CPUs=1 Sockets=1 CoresPerSocket=1 ThreadsPerCore=1 RealMemory=1945 State=UNKNOWN
PartitionName=base Nodes=worker Default=Yes MaxTime=72:00:00 Priority=1 State=UP
#PartitionName=cache Nodes=worker,controller MaxTime=72:00:00 Priority=1 State=UP
#PartitionName=cache2 Nodes=compute-slurm2 MaxTime=72:00:00 Priority=1 State=UP
# DefMemPerNode=1000
# MaxMemPerNode=1000
# DefMemPerCPU=4000
# MaxMemPerCPU=4096
